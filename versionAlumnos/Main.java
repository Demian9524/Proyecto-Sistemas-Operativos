...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 1; i <= n; i++) {
            ...
            int prio = leerEnteroNoNegativo(sc, "Prioridad (entero, puede ser 0): ");

            Proceso p = new Proceso(id, nombre, sizeKB, cpuTotal, llegada, prio);
            todos.add(p);
            usados.add(id);
        }
        ...
        System.out.println("\n====== PROCESOS INGRESADOS ======");
        for (Proceso p : todos) {
            System.out.printf("ID: %-8s  Nom: %-10s  Size=%4dKB  CPU=%5dms  Llegada=%5dms  Prio=%d%n",
                    p.getId(), p.getNombre(), p.getSizeKB(), p.getCpuTotalMs(), p.getLlegadaMs(), p.getPrioridad());
        }
        ...
        while (true) {
            ...
            // (A) Llegadas al tiempo actual (imprime AMBAS colas al momento de llegada)
            idxLleg = procesarLlegadas(todos, idxLleg, reloj, colaListos, colaMemoria);

            // (B) Cargar a memoria desde "Procesos Listos" (FIFO estricta)
            cargarMemoriaFIFO(mem, reloj, colaListos, colaMemoria);

            // (C) Ejecutar RR si hay procesos en memoria (granular: 1 ms por iteración)
            if (!colaMemoria.estaVacia()) {
                ...
                // Fin del turno del proceso actual
                if (actual.terminado()) {
                    mem.liberar(actual, reloj);
                    Logs.termina(reloj, actual);
                    // No regresa a colas
                } else {
                    // Expiró quantum: sale de memoria y regresa a "Procesos Listos"
                    mem.liberar(actual, reloj);
                    Logs.quantumExpira(reloj, actual);
                    colaListos.encolar(actual);
                    Logs.cola("Procesos Listos", colaListos);
                }
                continue; // iterar: podrían entrar nuevas llegadas en este mismo reloj
            }
    ...
    // Carga de "Procesos Listos" -> "Listos para Ejecución" respetando FIFO estricta; imprime cada cambio
    private static void cargarMemoriaFIFO(Memoria mem, int reloj,
        ...
        while (cargo && !colaListos.estaVacia()) {
            ...
            if (mem.cabe(f)) {
                ...
                cargo = false; // NO saltar al siguiente (FIFO estricta)
            }
        }
    }
